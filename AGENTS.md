...
- 私密资料遵循用户全局规则：密钥只放 ~/.codex/env/private.env，文件 600、目录 700；不回显、不复制入仓库。项目 .env 保存本项目配置与报名资料，不提交 Git；模型密钥不放入项目。
...
- 仓库及成果站公开后，submission.md 只保留公开摘要；报名资料和表单入口保存在被 Git 忽略的项目 .env；账号会话与完整查询记录保存在仓库外的本机私密目录，均不写进 Git。新增公开内容需检查个人信息和凭据。
- 报名固定资料通过 submission_config.py 按变量白名单从本项目 .env 或进程环境读取，不读取或回退到全局 private.env；完整变量示例见 .env.example。检查只显示配置状态，不回显字段值，不把配置就绪当成已报名或已提交。

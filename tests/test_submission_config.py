...
class SubmissionConfigTests(unittest.TestCase):
    ...
    def load(self, env=None):
        return config.load_profile(env or {}, self.project)

    def test_scoped_read_and_precedence(self):
        self.project.write_text(self.project.read_text() + 'EASYDATA_COURSE_NAME="Fixture course"\nEASYDATA_COURSE_RATING=3\n')
    ...
    def test_no_evaluation_and_correct_task_link(self):
        ...
        self.project.write_text(self.project.read_text() + 'EASYDATA_NOTES_URL_TEMPLATE=https://example.com/task{task}/\n')
    ...
    def test_project_isolation_and_no_global_private_fallback(self):
        other = self.root / "other.env"
        other.write_text('EASYDATA_WECHAT_NICKNAME=another-project\n')
        with patch.object(Path, "home", side_effect=AssertionError("Global home must not be consulted")):
            first = self.load()
            second = config.load_profile({}, other)
            missing = config.load_profile({}, self.root / "missing.env")
        self.assertEqual(first["EASYDATA_WECHAT_NICKNAME"], "fixture nickname")
        self.assertEqual(second["EASYDATA_WECHAT_NICKNAME"], "another-project")
        self.assertNotIn("EASYDATA_TEAM_NAME", second)
        self.assertNotIn("EASYDATA_WECHAT_NICKNAME", missing)
        with self.assertRaises(ValueError):
            config.validate_profile(missing)
    ...
    def test_cli_never_echoes_personal_values_on_success_or_failure(self):
        for malformed in (False, True):
            if malformed:
                self.project.write_text('EASYDATA_WECHAT_NICKNAME="fixture-private-unclosed\n')
            output = io.StringIO()
